...
for ebno = 1:length(EbNo) %% Running loop to run simulation for each SNR value
    ...
    while number_of_bits < maximum_number_of_bits %%% Running loop to process one frame in each iteration
        ...
        %%% OSTBC Encoding
        if number_of_transmit_antenna == 2
            modulated_data((2:2:end).') = -conj(modulated_data((2:2:end).'));
            ostbc_encoded_data = ostbc_encoder(modulated_data);
            ostbc_encoded_data = ostbc_encoded_data / sqrt(2);
        elseif number_of_transmit_antenna == 4
            modulated_data_length = size(modulated_data, 1);
            ostbc_encoded_data = complex(zeros(modulated_data_length, 4));
            modulated_data((2:2:end).') = -conj(modulated_data((2:2:end).'));
            index_12 = ([1:4:modulated_data_length; 2:4:modulated_data_length]);
            index_12 = index_12(:);
            index_34 = ([3:4:modulated_data_length; 4:4:modulated_data_length]);
            index_34 = index_34(:);
            ostbc_encoded_data(index_12, [1 3]) = ostbc_encoder(modulated_data(index_12));
            ostbc_encoded_data(index_34, [2 4]) = ostbc_encoder(modulated_data(index_34));
            ostbc_encoded_data = ostbc_encoded_data / sqrt(2);
        end
        %%% OSTBC Encoding

        %%% Filling the resource blocks with data symbols and making spaces for pilot symbols
        if number_of_transmit_antenna == 1 && number_of_receive_antenna == 1
            ...
            reshaped_modulated_data = reshape(ostbc_encoded_data, number_of_resource_block * number_of_data_subcarrier, number_of_symbol_per_subcarrier, number_of_transmit_antenna);
        end
        ...
        %%% Preparing the ofdm demodulated data symbols for OSTBC Combiner
        ...
        %%% OSTBC Combining
        processed_ofdm_demodulated_data_length = size(processed_ofdm_demodulated_data, 1);
        index = (2:2:processed_ofdm_demodulated_data_length)';
        if number_of_transmit_antenna == 2
            processed_ofdm_demodulated_data = sqrt(2) * processed_ofdm_demodulated_data;
            ostbc_combined_data = ostbc_combiner(processed_ofdm_demodulated_data, channel_estimation_matrix);
            ostbc_combined_data(index) = -conj(ostbc_combined_data(index));
        elseif number_of_transmit_antenna == 4
            processed_ofdm_demodulated_data = sqrt(2) * processed_ofdm_demodulated_data;
            h = complex(zeros(processed_ofdm_demodulated_data_length, 2, number_of_receive_antenna));
            index12 = ([1:4:processed_ofdm_demodulated_data_length; 2:4:processed_ofdm_demodulated_data_length]);
            index12 = index12(:);
            index34 = ([3:4:processed_ofdm_demodulated_data_length; 4:4:processed_ofdm_demodulated_data_length]);
            index34 = index34(:);
            h(index12, :, :) = channel_estimation_matrix(index12, [1 3], :);
            h(index34, :, :) = channel_estimation_matrix(index34, [2 4], :);
            ostbc_combined_data = ostbc_combiner(processed_ofdm_demodulated_data, h);
            ostbc_combined_data(index) = -conj(ostbc_combined_data(index));
        end
        %%% OSTBC Combining
        ...
        if number_of_transmit_antenna == 1 && number_of_receive_antenna == 1
            ...
            demodulated_data = demodulator_hard(ostbc_combined_data); %% Demodulating
        end
